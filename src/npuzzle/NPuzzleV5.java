...
public class NPuzzleV5 {
    ...
    private static void puzzle(int num) {
        ...
        ArrayList<npuzzle.Tile> tiles = generate(DIM);
        ArrayList<npuzzle.Tile> ansKey = getKey(DIM);
    ...
    /**
     * Creates n*n - 1 Tile objects for the puzzle (see Tile class). If the given
     * combination of coordinates makes an unsolvable puzzle, a new set is
     * generated.
     * 
     * @param n the int which represents the dimensions of the puzzle generated, and
     *          controls the iterations of Tile creation; passed down from puzzle()
     * @return an ArrayList of Tiles which will store the data for each Tile object
     *         in the puzzle
     ...
     * @return an ArrayList of Tile objects containing the coordinates they will
     *         each have when puzzle is solved
     * @see #solved(ArrayList, ArrayList, int)
     */
    private static ArrayList<npuzzle.Tile> getKey(int n) {
        ArrayList<npuzzle.Tile> tiles = new ArrayList<>();
        for (int k = 1; k <= n * n - 1; k++) {
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < n; j++) {
                    int[] c = { i, j };
                    tiles.add(new npuzzle.Tile(k, n, c));
                    k++;
                }
            }
        }
    ...
    /**
     * method is run after every legal move input by player to sync new coordinates
     * to Tile objects and displayed puzzle matrix
     * 
     * @param board int[][] being updated to visualize the current coordinates of
     *              Tiles
     * @param tiles ArrayList of Tile objects where coordinates are synced to
     * @see #runGame(ArrayList, ArrayList, int[][], int[][], int)
     */
    private static void update(int[][] board, ArrayList<npuzzle.Tile> tiles) {
        for (npuzzle.Tile t : tiles) {
    ...
    /**
     * <p>
     * loop which accepts user input for moves until puzzle is solved. The user
     * inputs the number of the Tile they wish to move, and if there is an empty
     * adjacent square, the Tile is moved into that position.
     * </p>
     * 
     * @param tiles   ArrayList of Tile objects which store puzzle coordinate data
     * @param ansKey  ArrayList of Tile objects storing solved puzzle coordinates,
     *                to determine if puzzle has been solved
     * @param board   int[][] to visualize puzzle coordinates
     * @param key     int[][] which is used to display how solved puzzle will look
     * @param tileMax int passed to solved() to limit number of iterations;
     *                represents highest Tile number
     */
    private static void runGame(ArrayList<npuzzle.Tile> tiles, ArrayList<npuzzle.Tile> ansKey, int[][] board,
            int[][] key, int tileMax) {
        ...
        do {
            ...
            try {
                ...
                switch (input) {
                    case "help":
                        System.out.println("Welcome to the " + DIM
                                + " Puzzler! \n\n RULES: \n 1) Move the numbered tiles on the board \n\t until they are in order from 1-"
                                + (DIM * DIM - 1) + ". \n\t It will look like this: ");
                        printBoard(key);
                        System.out.println(
    ...
    /**
     * checks if one Tile ArrayList matches another; used to check current game
     * board vs answer key
     * 
     ...
     * @return true if the coordinates stored in curr Tiles and ans Tiles are the
     *         same
     */
    private static boolean solved(ArrayList<npuzzle.Tile> curr, ArrayList<npuzzle.Tile> ans, int max) {
        int n = 0;
        for (int i = 0; i < max; i++) {
            int x = curr.get(i).x();
            int y = curr.get(i).y();
